...
difference(){
    union(){
        ecube([unit*4, unit*10, unit], true);
        translate([0, 0, 1.5])
            ecube([unit*4, unit*8, unit+3], true);
    }
    for(y = [-1, 1])
        translate([-unit*1.5, y*unit*4.5, 0])
        holes(4);
    ...
    for (i = [-1,1]){
        translate([15/2*i, 56/2, 15/2-unit/2+1])
            rotate([90, 0, 0])
            unipolar_contact();
    }
